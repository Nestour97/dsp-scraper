...
# ------------------------------------------------------------
# Spotify scraper (your logic unchanged)
# Only changes vs Colab:
#   - removed Colab-only imports (nest_asyncio, google.colab.files)
#   - run() returns the absolute path to the Excel file
#   - added Streamlit wrapper: run_spotify_scraper(test_mode, test_countries)
#   - safe event-loop handling for Streamlit (thread fallback)
# ------------------------------------------------------------
...
def extract_amount_number(text):
    ...
    m = re.search(r"(?:after|then|per\s+month|monthly|month)\D{0,12}(\d+(?:[.,]\d+)?)", tr, re.I)
    if m and not re.search(r"hour|hours|hr|hrs|minute|min", m.group(0), re.I):
        return _normalize_number(m.group(1))

    if is_generic_trial(t):
        m2 = re.search(r"(?:for\s+1\s+month|trial|free\s+for\s+\d+\s+month(?:s)?)\D{0,12}(\d+(?:[.,]\d+)?)", tr, re.I)
...
async def run():
    async with async_playwright() as pw:
        ...
        if not all_plans:
            log("❌ No plan cards scraped. Try again (Spotify throttling sometimes).")
            return None
...
# ---------------------------------------------------------------------------
# Streamlit wrapper (THIS is what your app calls)
# ---------------------------------------------------------------------------
async def _run_spotify_async(test_mode: bool = True, test_countries=None) -> str | None:
    ...
    # Streamlit passes alpha-2 codes like ["AE","FR"] in test mode
    if TEST_MODE and test_countries:
        TEST_MARKETS = [c.lower() for c in test_countries if c and len(c.strip()) == 2]
        print(f"[SPOTIFY] UI-driven TEST_MARKETS: {TEST_MARKETS}", flush=True)

    # In full mode ignore test list; scrape all markets
    if not TEST_MODE:
        TEST_MARKETS = []
...
def run_spotify_scraper(test_mode: bool = True, test_countries=None) -> str | None:
    """
    Entry point used by Streamlit app via dsp_scrapers.run_scraper().
    Returns absolute path to the Excel file.
    Handles "event loop already running" by running in a thread.
    """
    try:
        return asyncio.run(_run_spotify_async(test_mode=test_mode, test_countries=test_countries))
    except RuntimeError as e:
        # If Streamlit/environment already has a running loop
        ...
        def runner():
            try:
                holder["path"] = asyncio.run(_run_spotify_async(test_mode=test_mode, test_countries=test_countries))
...
if __name__ == "__main__":
    # Example:
    #   python -m dsp_scrapers.spotify_scraper
    #   python -m dsp_scrapers.spotify_scraper
